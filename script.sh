...
for protocol in udp tcp; do
	...
	if [ "$bool" == "false" ] 
	  then openDNS=$(dig +short @208.67.222.222 -x ${AddressB})
	    if [ -z "$openDNS" ]
	      then openDNS="null"
	    fi
	  cloudflareDNS=$(dig +short @1.1.1.1 -x ${AddressB})
	    if [ -z "$cloudflareDNS" ]
	      then cloudflareDNS="null"
	    fi
	 adguardDNS=$(dig +short @176.103.130.130 -x ${AddressB})
	    if [ -z "$adguardDNS" ]
	      then adguardDNS="null"
	    fi

	       whois=$(whois ${AddressB} | grep -i "org-name" | cut -f2 -d:)
	       if [ -z "$whois" ]
		 then  whois=$(whois ${AddressB} | grep -i "OrgName" | cut -f2 -d:)
 		   if [ -z "$whois" ]
			then  whois=$(whois 5.9.13.67 | grep -i "descr" | head -n 1 | cut -f2 -d:)
		      if [ -z "$whois" ]
			then  whois="null"
		      fi
	           fi
		 ...
		 then sni="null"
	       ...
	       addIP $AddressB "$openDNS" "$cloudflareDNS" "$adguardDNS" "$whois" $sni 
	fi

	for x in "${IP[@]}"; do
	     if [ "$x" == "$AddressB" ]      
		then printf ",\"${value[${x}_openDNS]}\",\"${value[${x}_cloudflareDNS]}\",\"${value[${x}_adguardDNS]}\",\"${value[${x}_whois]}\",\"${value[${x}_sni]}\"\n"
		break
	     fi
	done
