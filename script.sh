...
tshark -r 2020_07_07_180408.pcap -E separator=, -T fields -e ip.src -e tcp.srcport -e ip.dst -e tcp.dstport -e ssl.handshake.extensions_server_name ssl.handshake.extensions_server_name > sni 
...
for protocol in udp tcp; do
    ...
    while IFS=' ' read -r AddressA_PortA f2 AddressB_PortB B_A_Frames B_A_Bytes A_B_Frames A_B_Bytes Total_Frames Total_bytes f10; do
        printf "${AddressA_PortA//':'/,},${AddressB_PortB//':'/,},$B_A_Frames,$B_A_Bytes,$A_B_Frames,$A_B_Bytes,$Total_Frames,$Total_bytes"
	...
	sni=$(cat sni | grep ${AddressB} | grep ${PortA} | cut -f5 -d',')
        if [ -z "$sni" ]; then
            sni="null"
	fi
	printf "$sni\n"
...
echo -e "Tempo di elaborazione: $(($ELAPSED_TIME%60))s"
